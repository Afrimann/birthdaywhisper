...
const THEMES = [
  { id: "CLASSIC",        label: "Classic",        bg: "from-[#1a1a2e] to-[#16213e]",   accent: "bg-[#ED93B1]",       border: "border-[#ED93B1]/40" },
  { id: "GOLDEN_HOUR",   label: "Golden Hour",    bg: "from-[#2d1b00] to-[#5c3200]",   accent: "bg-[#FAC775]",       border: "border-[#FAC775]/40" },
  { id: "MIDNIGHT_STARS",label: "Midnight Stars", bg: "from-[#0a0015] to-[#1a0030]",   accent: "bg-purple-400",      border: "border-purple-400/40" },
  { id: "BLOOM",          label: "Bloom",          bg: "from-[#1a0012] to-[#3d0030]",   accent: "bg-[#FB7185]",       border: "border-[#FB7185]/40" },
...
export default function MessageForm({ recipientId, recipientName, birthdayYear, isToday, isSignedIn }: Props) {
  ...
  if (needsAccount) {
    return (
      <div className="glass rounded-2xl p-8 text-center animate-fade-rise">
        ...
        <p className="text-accent-700 text-sm mb-6">
  ...
  return (
    <div className="glass rounded-2xl p-6 animate-fade-rise" style={{ animationDelay: "60ms" }}>
      ...
      {/* Card theme picker */}
      <div className="mb-4">
        <p className="text-xs text-accent-700 mb-2">Card theme</p>
          ...
          {THEMES.map((t) => (
            <button
              ...
              className={`relative h-10 rounded-lg bg-gradient-to-br ${t.bg} border transition-all ${
                cardTheme === t.id ? `${t.border} ring-2 ring-offset-1 ring-offset-canvas ring-current scale-105` : "border-blush hover:scale-105"
              }`}
            >
      ...
      >
        {isAnonymous
          ? <EyeOff className="w-4 h-4 text-accent-500 flex-shrink-0" />
          : <User className="w-4 h-4 text-accent-700 flex-shrink-0" />
        }
        <div>
          <p className="text-sm font-medium text-accent-900 leading-none mb-0.5">
