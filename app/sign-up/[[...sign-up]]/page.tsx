...
export default function SignUpPage() {
  ...
  // Deliberately NOT async: awaiting before sso() breaks iOS Safari's
  // user-gesture context and silently blocks the redirect. Errors are
  // handled in .then() — if the redirect succeeds the page unloads anyway.
    ...
    void signUp
      ...
      .then(({ error: ssoErr }) => {
        if (ssoErr) {
          ...
          setError(ssoErr.longMessage ?? ssoErr.message ?? "Google sign-up failed. Please try again.");
        }
      })
      .catch(() => {
        setGoogleLoading(false);
    ...
    <div className="min-h-screen bg-canvas flex flex-col items-center justify-center px-4">
      ...
      <div className="relative z-10 mb-8 text-center animate-fade-rise">
        <Link href="/" className="inline-flex items-center gap-2 mb-4">
          ...
          <span className="font-fraunces text-2xl font-bold text-accent-900 tracking-tight">
            BirthdayWhisper
          </span>
        </Link>
        {step === "create" ? (
          <>
            <h1 className="font-fraunces text-xl font-bold text-accent-900 mb-1">
              Create your account
            </h1>
            <p className="text-accent-700 text-sm">Your birthday page is waiting</p>
          </>
        ) : (
          <>
            <h1 className="font-fraunces text-xl font-bold text-accent-900 mb-1">
            ...
            <p className="text-accent-700 text-sm">
              ...
              <span className="text-accent-900 font-medium">{email}</span>
            </p>
          </>
        )}
      </div>
            ...
            {i === 0 && (
              <div
                className={`w-8 h-px transition-all ${
                  step === "verify" ? "bg-accent-500" : "bg-blush"
                }`}
              />
            )}
          </div>
        ...
        <div className="card rounded-xl p-8">
          {step === "create" ? (
            <form onSubmit={handleCreate} className="space-y-5">
              {/* Email */}
              <div>
                <label className="block text-accent-700 text-xs font-semibold uppercase tracking-wider mb-2">
                  Email
                </label>
                <input
                  type="email"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-ghost hover:text-accent-700 transition-colors"
                  >
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-accent-500 hover:bg-accent-600 disabled:opacity-50 disabled:cursor-not-allowed text-canvas font-semibold py-3 rounded-xl transition-all min-h-[44px]"
              >
                {isLoading ? (
            ...
            <form onSubmit={handleVerify} className="space-y-5">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 text-accent-700 hover:text-accent-900 text-sm transition-colors min-h-[44px]"
              >
            ...
            <>
              <div className="mt-5">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 bg-[rgba(75,21,40,0.05)] hover:bg-[rgba(75,21,40,0.09)] active:bg-[rgba(75,21,40,0.12)] border border-[rgba(75,21,40,0.1)] hover:border-[rgba(75,21,40,0.18)] text-accent-900 font-medium py-3 rounded-xl transition-all min-h-[44px] text-sm touch-manipulation disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                >
                  {googleLoading ? (
                    <>
